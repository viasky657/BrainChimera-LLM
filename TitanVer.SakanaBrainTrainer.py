...
# Configuration for Hierarchical Episodic Memory
class EpisodicMemoryConfig:
    def __init__(self, capacity=10000, hdbscan_min_cluster_size=5, hdbscan_metric='euclidean',
                 hdbscan_cluster_selection_epsilon=0.0, som_grid_size=10, som_input_dim=512,
                 som_sigma=1.0, som_learning_rate=0.5, cluster_method='hdbscan',
                 enable_cluster_maintenance=True, cluster_maintenance_interval=100,
                 cluster_centroid_update_freq=10, cluster_relevance_threshold=0.7,
                 max_cluster_size=50, merge_similarity_threshold=0.9, hierarchical_clustering_method='hdbscan'):
...
class HierarchicalEpisodicMemory:
    def __init__(self, capacity, config, cluster_method='hdbscan'):
        self.capacity = capacity
        self.memory = []  # List to store experiences
        self.cluster_method = cluster_method
        self.position = 0
        self.config = config
        self.next_cluster_id = 0  # Initialize a counter for cluster IDs
        self.clusterer = None  # Placeholder for the clustering model
        self.cluster_map = {}  # Map cluster IDs to experience indices
        self.current_neuron_count = 0  # Keeps track of the number of neurons
        self.cluster_centroids = {}  # Store cluster centroids
        self.cluster_members = {}  # Store cluster members
        self.hierarchy = {}  # Store the hierarchy of clusters
        self.clusters = {} # I have added this code to fix an error.

        # Threading for background clustering
        self.clustering_thread = None
        self.stop_clustering_thread = False
        ...
        elif self.cluster_method == 'som':
            self.som = MiniSom(x=config.som_grid_size, y=config.som_grid_size, input_len=config.som_input_dim,
                               sigma=config.som_sigma, learning_rate=config.som_learning_rate)

    def push(self, experience):
        """Adds an experience to the memory and assigns it to a cluster."""
        if len(self.memory) < self.capacity:
            self.memory.append(None)
        self.memory[self.position] = experience
        self.position = (self.position + 1) % self.capacity

        # Add the experience to the data queue for background clustering
        self.data_queue.put(experience)

    def start_clustering_thread(self):
        """Starts the background clustering thread."""
        if self.clustering_thread is None or not self.clustering_thread.is_alive():
            self.stop_clustering_thread = False
            self.clustering_thread = threading.Thread(target=self.background_clustering)
            self.clustering_thread.daemon = True  # Allow program to exit even if thread is running
            self.clustering_thread.start()

    def stop_clustering_thread(self):
        """Stops the background clustering thread."""
        self.stop_clustering_thread = True
        if self.clustering_thread is not None and self.clustering_thread.is_alive():
            self.clustering_thread.join()

    def background_clustering(self):
      """
      Performs clustering in the background.
      """
      while not self.stop_clustering_thread:
          try:
              experience = self.data_queue.get(timeout=1)  # Wait for 1 second for new data
              if experience is not None:
                  self.cluster_experience(experience)
                  self.data_queue.task_done()
          except queue.Empty:
              # If the queue is empty, check for cluster maintenance
              if self.config.enable_cluster_maintenance:
                  self.perform_cluster_maintenance()
              continue  # Continue to the next iteration to check stop condition
          except Exception as e:
              print(f"Error during background clustering: {e}")
              # Handle other potential errors during clustering or maintenance

    def cluster_experience(self, experience):
        """Clusters the experience based on goal/context embeddings using the specified method."""
        goal_context_embedding = torch.cat([experience['goal'], experience['context']], dim=-1).cpu().detach().numpy()

        if self.cluster_method == 'hdbscan':
            self.update_hdbscan_clustering(goal_context_embedding, experience)
        elif self.cluster_method == 'som':
            self.perform_som_clustering(goal_context_embedding)
        else:
            self.perform_kmeans_clustering(experience, goal_context_embedding)

        # Check if it's time to perform cluster maintenance
        if self.config.enable_cluster_maintenance and len(self.memory) % self.config.cluster_maintenance_interval == 0:
            self.perform_cluster_maintenance()

    def update_hdbscan_clustering(self, new_embedding, experience):
        """
        Updates the HDBSCAN clustering with a new data point (embedding).
        """
        # Check if the clusterer has been fitted with data before
        if self.clusterer.labels_ is None:
            # Fit the clusterer with the initial data point
            self.clusterer.fit(new_embedding.reshape(1, -1))
            # Assign the first cluster ID (0 or -1 for noise)
            cluster_id = self.clusterer.labels_[0] if self.clusterer.labels_[0] != -1 else self.next_cluster_id
            if cluster_id == self.next_cluster_id:
                self.next_cluster_id += 1
        else:
            # Update the clusterer with the new data point
            cluster_id = hdbscan.prediction.approximate_predict(self.clusterer, [new_embedding])[0][0]

        # Assign the experience to the appropriate cluster
        if cluster_id not in self.clusters:
            self.clusters[cluster_id] = {'members': []}
        self.clusters[cluster_id]['members'].append(experience)

        # Update cluster centroids (if needed)
        self.update_cluster_centroids()

    def perform_som_clustering(self, goal_context_embedding: torch.Tensor):
        """
        Assigns the experience to a cluster based on SOM.
        """
        # Train SOM with the new embedding
        self.som.train_batch(goal_context_embedding.unsqueeze(0).cpu().numpy(), num_iteration=1)

        # Find the best matching unit (BMU) for the new experience
        bmu = self.som.winner(goal_context_embedding.cpu().numpy())

        # Assign the experience to the cluster corresponding to the BMU
        cluster_id = str(bmu)  # Convert tuple to string to use as dictionary key
        if cluster_id not in self.clusters:
            self.clusters[cluster_id] = {'centroid': goal_context_embedding, 'members': []}

        self.clusters[cluster_id]['members'].append(self.memory[self.position - 1])

        # Update the centroid of the cluster (simple average for demonstration)
        cluster_embeddings = [torch.cat([mem['goal'], mem['context']], dim=-1) for mem in
                              self.clusters[cluster_id]['members']]
        self.clusters[cluster_id]['centroid'] = torch.stack(cluster_embeddings).mean(dim=0)

    def update_cluster_centroids(self):
        """
        Updates the centroids of all clusters based on their current members.
        """
        for cluster_id, cluster_data in self.clusters.items():
            if cluster_data['members']:
                # Recalculate the centroid as the mean of all member embeddings
                member_embeddings = [torch.cat([mem['goal'], mem['context']], dim=-1) for mem in cluster_data['members']]
                new_centroid = torch.stack(member_embeddings).mean(dim=0)
                self.clusters[cluster_id]['centroid'] = new_centroid

    def perform_kmeans_clustering(self, experience, goal_context_embedding):
      """Assigns the experience to the nearest cluster based on k-means."""
      if not self.clusters:
          # Initialize with the first experience
          self.clusters[self.next_cluster_id] = {'centroid': goal_context_embedding, 'members': [experience]}
          self.next_cluster_id += 1
      else:
          ...
          for cluster_id, cluster_data in self.clusters.items():
              dist = torch.norm(goal_context_embedding - cluster_data['centroid'])
              if dist < min_dist:
                  min_dist = dist
                  closest_cluster_id = cluster_id

          # Add the experience to the closest cluster
          self.clusters[closest_cluster_id]['members'].append(experience)

          # Recompute the centroid (you might want to do this periodically, not every time)
          if len(self.clusters[closest_cluster_id]['members']) % self.config.cluster_centroid_update_freq == 0:
              cluster_embeddings = [torch.cat([mem['goal'], mem['context']], dim=-1) for mem in
                                    self.clusters[closest_cluster_id]['members']]
              self.clusters[closest_cluster_id]['centroid'] = torch.stack(cluster_embeddings).mean(dim=0)

    def retrieve(self, goal: torch.Tensor, context: torch.Tensor, num_memories: int):
      """
      Retrieves relevant memories based on the current goal and context using a hierarchical approach.
      """
      # 1. Find relevant cluster(s) based on goal and context
      relevant_cluster_ids = self.find_relevant_clusters(goal, context)

      # 2. Retrieve memories from the relevant clusters
      relevant_memories = []
      for cluster_id in relevant_cluster_ids:
          relevant_memories.extend(self.clusters[cluster_id]['members'])

      # 3. Rank memories by similarity to the current goal and context
      if relevant_memories:
          goal_context_embedding = torch.cat([goal, context], dim=-1)
          memory_embeddings = torch.stack([torch.cat([m['goal'], m['context']], dim=-1) for m in relevant_memories])
          similarity_scores = F.cosine_similarity(goal_context_embedding.unsqueeze(0), memory_embeddings, dim=1)

          # Sort memories by similarity score (descending order)
          sorted_indices = torch.argsort(similarity_scores, descending=True)
          sorted_memories = [relevant_memories[i] for i in sorted_indices]

          # Select the top-k most similar memories
    ...
    def find_relevant_clusters(self, goal: torch.Tensor, context: torch.Tensor) -> List[int]:
      """
      Finds clusters relevant to the current goal and context based on embedding similarity.
      """
      goal_context_embedding = torch.cat([goal, context], dim=-1)
      relevant_clusters = []

      for cluster_id, cluster_data in self.clusters.items():
          # Calculate cosine similarity between the combined goal-context embedding
          # and the cluster centroid
          similarity = F.cosine_similarity(
              goal_context_embedding,
              cluster_data['centroid'].unsqueeze(0),
              dim=1
          )

          # If similarity is above a threshold, consider it relevant
          if similarity.item() > self.config.cluster_relevance_threshold:
              relevant_clusters.append(cluster_id)

      # If no clusters are found, return the closest cluster (fallback)
      if not relevant_clusters:
          ...
          for cluster_id, cluster_data in self.clusters.items():
              ...
              if dist < min_dist:
                  min_dist = dist
                  closest_cluster_id = cluster_id
          relevant_clusters.append(closest_cluster_id)

      return relevant_clusters
    
    def get_k_nearest_memories(self, query_embedding: torch.Tensor, k: int) -> List[Dict]:
        """
        Retrieves the k nearest memories to the given query embedding based on cosine similarity.
        """
        if not self.memory:
            return []

        # Calculate cosine similarity between query embedding and all memory embeddings
        memory_embeddings = torch.stack([torch.cat([m['goal'], m['context']], dim=-1) for m in self.memory])
        similarities = F.cosine_similarity(query_embedding.unsqueeze(0), memory_embeddings, dim=1)

        # Get indices of the k most similar memories
        _, top_indices = torch.topk(similarities, k)

        # Retrieve the corresponding memories
        nearest_memories = [self.memory[i.item()] for i in top_indices]

        return nearest_memories
    
    def perform_cluster_maintenance(self):
        """
        Performs maintenance on the clusters, such as splitting large clusters,
        merging similar clusters, and creating a hierarchy of clusters.
        """
        if not self.clusters:
            return
        ...
        for cluster_id, cluster_data in list(self.clusters.items()):
            if len(cluster_data['members']) > max_cluster_size:
                self.split_cluster(cluster_id)
        
        # Merge similar clusters if necessary
        self.merge_similar_clusters(self.config.merge_similarity_threshold)
    ...
    def split_cluster(self, cluster_id: int):
      """
      Splits a large cluster into smaller clusters.
      """
      if cluster_id not in self.clusters:
          print(f"Cluster {cluster_id} not found.")
          return
      
      cluster_members = self.clusters[cluster_id]['members']
      num_members = len(cluster_members)

      if num_members < 2:
        print(f"Cluster {cluster_id} has too few members to split.")
        return

      # Use a simple clustering algorithm like KMeans for splitting
      embeddings = [torch.cat([m['goal'], m['context']], dim=-1).cpu().numpy() for m in cluster_members]
      embeddings = np.array(embeddings)  # Convert list of embeddings to a NumPy array

      n_clusters = min(num_members, max(2, int(num_members / self.config.max_cluster_size)))  # Example heuristic
      ...
      try:
          ...
          print(f"Number of samples: {embeddings.shape[0]}, number of features: {embeddings.shape[1]}")
          return

      new_cluster_ids = [self.next_cluster_id + i for i in range(n_clusters)]
      self.next_cluster_id += n_clusters

      # Create new clusters
      for new_id in new_cluster_ids:
          self.clusters[new_id] = {'centroid': None, 'members': []}

      # Assign members to new clusters based on KMeans labels
      for i, member in enumerate(cluster_members):
          new_cluster_id = new_cluster_ids[kmeans.labels_[i]]
          self.clusters[new_cluster_id]['members'].append(member)

      # Remove the old cluster
      del self.clusters[cluster_id]

      # Update centroids for new clusters
      self.update_cluster_centroids()

    def merge_similar_clusters(self, threshold: float):
        """
        Merges clusters that are similar based on a similarity threshold.
        """
        cluster_ids = list(self.clusters.keys())
        if len(cluster_ids) < 2:
          return
        
        merged = [False] * len(cluster_ids)  # Keep track of which clusters have been merged
        
        for i, cluster_id1 in enumerate(cluster_ids):
          if merged[i]:
              continue  # Skip if already part of a merged cluster
          for j, cluster_id2 in enumerate(cluster_ids[i+1:], start=i+1):
              if merged[j]:
                  continue  # Skip if already part of a merged cluster
              
              # Calculate cosine similarity between centroids
              centroid1 = self.clusters[cluster_id1]['centroid']
              centroid2 = self.clusters[cluster_id2]['centroid']
              similarity = F.cosine_similarity(centroid1.unsqueeze(0), centroid2.unsqueeze(0))
              
              if similarity.item() > threshold:
                # Merge cluster2 into cluster1
                self.clusters[cluster_id1]['members'].extend(self.clusters[cluster_id2]['members'])
                
                # Update centroid of cluster1
                cluster_embeddings = [torch.cat([mem['goal'], mem['context']], dim=-1) for mem in
                                      self.clusters[cluster_id1]['members']]
                self.clusters[cluster_id1]['centroid'] = torch.stack(cluster_embeddings).mean(dim=0)

                # Mark cluster2 as merged
                merged[j] = True

        # Remove merged clusters (those marked as True in merged list)
        for i in range(len(merged) - 1, -1, -1):
            if merged[i]:
                del self.clusters[cluster_ids[i]]

    def create_cluster_hierarchy(self):
        """
        Creates a hierarchy of clusters using a chosen clustering algorithm (e.g., HDBSCAN).
        """
        # Extract all current cluster centroids
        centroid_embeddings = [data['centroid'] for data in self.clusters.values()]
        centroid_embeddings_np = torch.stack(centroid_embeddings).cpu().detach().numpy()

        if self.config.hierarchical_clustering_method == 'hdbscan':
            # Apply HDBSCAN for hierarchical clustering
            ...
            clusterer.fit(centroid_embeddings_np)
            labels = clusterer.labels_

            # Organize the hierarchy
            self.hierarchy = {}
            for cluster_id, label in zip(self.clusters.keys(), labels):
                if label != -1:  # Ignore noise points
                    if label not in self.hierarchy:
                        self.hierarchy[label] = []
                    self.hierarchy[label].append(cluster_id)

        else:
            raise ValueError(f"Unsupported hierarchical clustering method: {self.config.hierarchical_clustering_method}")
    ...
    def calculate_silhouette_score(self, embeddings, labels):
        ...
        Args:
            embeddings: A numpy array of embeddings used for clustering.
            labels: A numpy array of cluster labels for each embedding.

        Returns:
            The silhouette score (float).
        """
        if len(set(labels)) <= 1:
            return -1  # Cannot calculate silhouette for single cluster or all noise

        score = silhouette_score(embeddings, labels)
        return score

    def get_cluster_size_distribution(self, labels):
        """
        Calculates the distribution of cluster sizes.

        Args:
            labels: A numpy array of cluster labels.

        Returns:
            A dictionary where keys are cluster labels and values are their sizes.
        """
    ...
    def calculate_intra_cluster_similarity(self, embeddings, labels):
        """
        ...
        Returns:
            The average intra-cluster similarity (float).
        """
        total_similarity = 0
        count = 0
          ...
          if len(cluster_indices) > 1:
            for i in range(len(cluster_indices)):
                for j in range(i + 1, len(cluster_indices)):
                    similarity = F.cosine_similarity(embeddings[cluster_indices[i]].unsqueeze(0),
                                                     embeddings[cluster_indices[j]].unsqueeze(0))
                    total_similarity += similarity.item()
                    count += 1
        return total_similarity / count if count > 0 else 0

    def calculate_inter_cluster_similarity(self, embeddings, labels):
        """
        Calculates the average inter-cluster similarity.
        ...
        Returns:
            The average inter-cluster similarity (float).
        """
        total_similarity = 0
        count = 0
        ...
        for label1 in unique_labels:
            ...
            for label2 in unique_labels:
                if label1 != label2:
                    ...
                    for i in indices1:
                        for j in indices2:
                            similarity = F.cosine_similarity(embeddings[i].unsqueeze(0),
    ...
    def evaluate_clustering(self, embeddings):
            ...
            """
            labels = self.get_cluster_labels()  # You need to implement this to get cluster labels for all experiences

            if len(set(labels)) > 1:
              metrics = {
                  'silhouette_score': self.calculate_silhouette_score(embeddings, labels),
                  ...
                  'intra_cluster_similarity': self.calculate_intra_cluster_similarity(embeddings, labels),
                  'inter_cluster_similarity': self.calculate_inter_cluster_similarity(embeddings, labels),
              }
            else:
              metrics = {
                'silhouette_score': -1,
            ...
            cluster_sizes = self.get_cluster_size_distribution(labels)
            metrics['cluster_size_distribution'] = cluster_sizes
            
            return metrics
        
    def get_cluster_labels(self):
      """
      Assigns each experience in the memory to a cluster and returns the cluster labels.
      This method assumes that each experience is associated with a cluster ID.
      """
      labels = []
      for experience in self.memory:
            if experience is not None:
                # Find the cluster ID for the experience
                cluster_id = None
                for cid, cluster_data in self.clusters.items():
                    if experience in cluster_data['members']:
                        cluster_id = cid
                        break
                labels.append(cluster_id if cluster_id is not None else -1)  # Use -1 for experiences not in any cluster
            else:
                labels.append(-1)  # Use -1 for empty slots in the memory
